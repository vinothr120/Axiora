...
function CopyableCode({ text }) {
  ...
  return (
    <button
      ...
      className="group -mx-1 inline-flex items-center gap-1.5 rounded px-1 py-0.5 hover:bg-slate-100 dark:hover:bg-slate-800"
    >
      <span>{text}</span>
      {copied ? (
        <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 shrink-0 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <polyline points="20 6 9 17 4 12" />
        </svg>
      ) : (
        <svg
          viewBox="0 0 24 24"
          className="h-3.5 w-3.5 shrink-0 text-slate-400 opacity-60 group-hover:opacity-100 dark:text-slate-500"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
                ...
                <tbody>
                  {filtered.map((c) => (
                    <tr key={c.id} className="odd:bg-white even:bg-slate-50/60 dark:odd:bg-slate-900 dark:even:bg-slate-800/40">
                      <td className="figure whitespace-nowrap px-2 py-2 font-medium text-slate-900 dark:text-white">
                        <CopyableCode text={c.code} />
                      </td>
