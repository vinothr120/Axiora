...
async function getGrant(codeId, templateId) {
  return db.get("SELECT * FROM code_templates WHERE code_id = ? AND template_id = ?", [codeId, templateId]);
}
...
router.use("/:id", async (req, res, next) => {
  ...
  const grant = await getGrant(req.clientSession.code.id, tpl.id);
  if (!grant) return res.status(403).json({ error: "template_not_granted" });
  req.templateGrant = grant;
  next();
});
...
router.get("/:id", async (req, res) => {
  ...
  res.json({
    ...
    canManageRows: Boolean(req.templateGrant.can_manage_rows),
...
router.post("/:id/calculate", async (req, res) => {
  const tpl = getTemplate(req.params.id);
  if (!tpl) return res.status(404).json({ error: "not_found" });
  const canManageRows = Boolean(req.templateGrant.can_manage_rows);
  ...
  // Add/delete-row is a permission the admin grants per (code, template) — separate
  // from the always-on OHLC editing above. When it's off, the custom/hidden portions
  // of the save are pinned to whatever is already persisted (untouched by this
  // request) rather than just cleared — a client mid-edit on OHLC values still has
  // rows they added earlier (from before the permission was revoked, say) echoed back
  // in the request body, and clearing on every unrelated edit would silently delete
  // that state. Only genuinely new custom/hidden entries in the request are rejected.
  let sanitizedCustom = [];
  let sanitizedHidden = [];
  if (canManageRows) {
    ...
    sanitizedHidden = (Array.isArray(body.hidden) ? body.hidden : []).filter((k) => validKeys.has(k));
  } else {
    const existing = await loadOverrides(req.clientSession.code.id, tpl.id);
    sanitizedCustom = existing.custom;
    sanitizedHidden = [...existing.hidden];
  }
